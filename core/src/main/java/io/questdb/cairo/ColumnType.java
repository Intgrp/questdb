...
public final class ColumnType {
    ...
    /**
     * Converts an array type with weak dimensionality to strong dimensionality.
     * The element type and dimensionality remain the same, but the weak flag is removed.
     */
    public static int makeArrayDimsStrong(int arrayType) {
        assert isArray(arrayType) : "typeTag of arrayType is not ARRAY";
        return arrayType & ~TYPE_FLAG_ARRAY_WEAK_DIMS;
    }
    ...
    /**
     * Updates the dimensionality of an array type, preserving the weak/strong flag.
     */
    public static int updateArrayDimensionality(int arrayType, int newNDims) {
        assert isArray(arrayType) : "typeTag of arrayType is not ARRAY";
        short elemType = decodeArrayElementType(arrayType);
        boolean isWeak = isArrayWithWeakDims(arrayType);

        int newType = encodeArrayType(elemType, newNDims, true);
        return isWeak ? (newType | TYPE_FLAG_ARRAY_WEAK_DIMS) : newType;
    }
